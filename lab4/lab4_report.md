...
- **Cloud SQL (PostgreSQL)** на следующих этапах — надёжная реляционная база с поддержкой сложных запросов.
- **Vertex AI / Gemini** — удобная интеграция с Google Cloud и мощные AI-модели.
- **Cloud Load Balancer + CDN** — для распределения нагрузки и ускорения доставки контента на проде.

## Выводы

В рамках лабораторной работы была спроектирована инфраструктура AI-приложения на трёх этапах развития. 

Главный вывод: **не стоит сразу строить сложную и дорогую инфраструктуру**. На этапе MVP важно максимально быстро и дёшево проверить гипотезу. По мере роста пользователей и обратной связи от партнёров инфраструктура должна постепенно усложняться, добавляя надёжность, масштабируемость и производительность.
...
Mermaid-диаграммы можно легко открыть и отредактировать в draw.io (import → Mermaid).
